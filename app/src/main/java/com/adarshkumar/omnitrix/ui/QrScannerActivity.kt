...
/**
 * Watch QR scanner. The QR is never opened as a URL or executed.
 * After a QR containing a Bluetooth MAC is decoded, the user gets an explicit
 * CONNECT TO WATCH action which passes that verified MAC to the GATT explorer.
 */
class QrScannerActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<MaterialButton>(R.id.qrCopy).setOnClickListener { lastParsed?.let { copy(it.raw); toast("QR payload copied") } }
        findViewById<MaterialButton>(R.id.qrSave).setOnClickListener { lastParsed?.let { pairing.record(it); toast("Saved locally for diagnostics") } }
        ...
        connectButton.setOnClickListener { connectFromQr() }
    ...
    private fun showResult(raw: String) {
        ...
        lines += "\nNothing is opened automatically. Use CONNECT TO WATCH to start the BLE connection."
        candidatesView.text = lines.joinToString("\n")

        val hasMac = parsed.macCandidates.firstOrNull() != null
        connectButton.visibility = if (hasMac) View.VISIBLE else View.GONE
        connectButton.isEnabled = hasMac
        hint.visibility = View.GONE
        resultPanel.visibility = View.VISIBLE
    }

    private fun connectFromQr() {
        val mac = lastParsed?.macCandidates?.firstOrNull() ?: run {
            toast("No Bluetooth MAC address was found in this QR")
            return
        }
        ...
        pairing.lastPayload?.let { /* payload already recorded */ }
        startActivity(
            Intent(this, GattExplorerActivity::class.java)
                .putExtra(GattExplorerActivity.EXTRA_ADDRESS, mac)
        )
    }
