...
class ProtocolCodecTest {
    ...
    fun `encode produces documented header layout`() {
        val frame = ProtocolEncoder.encodeMoyoungHypothesis(
    ...
    fun `encode empty payload declares size 5`() {
        val frame = ProtocolEncoder.encodeMoyoungHypothesis(0x01)
    ...
    fun `round trip encode then decode`() {
        ...
        val encoded = ProtocolEncoder.encodeMoyoungHypothesis(0x42, payload)
        val decoded = ProtocolDecoder.decode(encoded)
    ...
    fun `decode rejects wrong magic`() {
        val result = ProtocolDecoder.analyze(byteArrayOf(0x00, 0x00, 0x20, 0x05, 0x31))
        assertTrue(result is ProtocolDecoder.Result.NotThisProtocol)
    }

    @Test
    fun `decode rejects truncated frame`() {
        val result = ProtocolDecoder.analyze(byteArrayOf(0xFE.toByte(), 0xEA.toByte(), 0x20))
        assertTrue(result is ProtocolDecoder.Result.Malformed)
    }
    ...
    fun `decode rejects declared size mismatch`() {
        ...
        val result = ProtocolDecoder.analyze(bad)
        assertTrue(result is ProtocolDecoder.Result.Malformed)
    }
    ...
    fun `decode rejects garbage command channel bytes`() {
        ...
        val result = ProtocolDecoder.analyze(garbage)
        assertTrue(result is ProtocolDecoder.Result.NotThisProtocol)
    }
    ...
    fun `legacy mtu20 framing is recognized`() {
        ...
        val result = ProtocolDecoder.analyze(legacy)
        assertTrue(result is ProtocolDecoder.Result.Valid)
        assertEquals(6, (result as ProtocolDecoder.Result.Valid).frame.declaredSize)
    }

    @Test
    fun `annotate never claims certainty`() {
        val frame = ProtocolEncoder.encodeMoyoungHypothesis(0x62, byteArrayOf(0))
        val note = ProtocolDecoder.annotate(frame)
